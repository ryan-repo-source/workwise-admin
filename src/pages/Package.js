...
function Package() {
    ...
    const GetFaqs = async () => {
        await axios.get('https://works.demo-customproject.com:4001/get-pricing', { headers: { 'Auth': localStorage.getItem('accessToken') } }).then((res) => {
            getFaq(res.data);
        })
    }
    ...
    const DelUser = async (id) => {
        if (window.confirm('Are you sure to delete this') === true) {
            await axios.delete(`https://works.demo-customproject.com:4001/delete-package/${id}`, { headers: { 'Auth': localStorage.getItem('accessToken') } }).then(function (response) {
                GetFaqs();
            })
        }
    }
            ...
            <div className="row">
                <div className="col-12">
                    <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 className="mb-sm-0 font-size-18"> Packages Management </h4>
                        <Link to="/admin/add-packges" className="btn btn-primary waves-effect waves-light"> Add New</Link>
                    </div>
                </div>
            </div>
            {/* end page title */}
                                        ...
                                        {faq.map((elem, key) => {
                                            return (
                                                <tr data-id={1} style={{ cursor: "pointer" }}>
                                                    ...
                                                    <td data-field="name" style={{ width: "30%" }}>
                                                        {elem.title}
                                                    </td>
                                                    <td data-field="name" style={{ width: "30%" }}>
                                                        ${elem.price}
