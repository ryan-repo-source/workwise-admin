...
const Sub_Categories = () => {
    ...
    const Getcategory = async () => {
        await axios.get('https://works.demo-customproject.com:4001/sub-categories', {headers : {'Auth' : localStorage.getItem('accessToken')}}).then((res) => {
            getCate(res.data);
        })
    }
    const DelCat = async (id) => {
        if (window.confirm('Are you sure to delete this') === true) {
            await axios.delete(`https://works.demo-customproject.com:4001/delete-cat-sub/${id}`, {headers : {'Auth' : localStorage.getItem('accessToken')}}).then(function (response) {
                Getcategory();
            })
        }
    }
                                        ...
                                        {Object.keys(cate).length === 0 ? <div className='notfound'>Not found</div> : cate.map((elem, key) => {
                                            return (
                                                <tr data-id={1} style={{ cursor: "pointer" }}>
                                                    ...
                                                    <td style={{ width: 140 }}>
                                                        <Link
                                                            to={`/admin/edit-sub-category/${elem.id}`}
